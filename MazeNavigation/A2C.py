...
class Net(nn.Module):
    ...
    def __init__(self, D_in, D_out):
        ...
        self.B1_p = nn.LayerNorm(D_width)
        self.L2_p = nn.Linear(D_width, D_out)

        # Define Value Network
        self.L1_v = nn.Linear(D_in, D_width)
        self.B1_v = nn.LayerNorm(D_width)
        self.L2_v = nn.Linear(D_width, 1)

    def forward(self, X):
        # Calculate Policy
        X_p = self.ReLU(self.B1_p(self.drop(self.L1_p(X))))
        X_p = self.soft(self.L2_p(X_p))

        # Calculate Value
        X_v = self.ReLU(self.B1_v(self.drop(self.L1_v(X))))
        X_v = self.L2_v(X_v)
...
for maze_choice in range(1, 2):
    for n_samples in [9]:
        ...
        maze_agent = Agent(8, 4, 0.001, discount=0.99, recent_memory=6, episode_length=500)
        ...
        print("\n FINAL Results", maze_choice, FixedStartFixedEnd, ", ", FixedStartMovingEnd, ", ", MovingStartMovingEnd, ", ", time.time() - T0)

        steps_to_goal_ittr = np.array(steps_to_goal_ittr)
        np.savetxt("A2C_" + str(n_samples) + ".csv", steps_to_goal_ittr, delimiter=",")
