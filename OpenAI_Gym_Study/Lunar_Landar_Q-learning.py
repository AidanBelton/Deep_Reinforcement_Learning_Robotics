...
class Net(nn.Module):
    ...
    def __init__(self, D_in, D_out):
        ...
        self.L2 = nn.Linear(D_width, D_width)
        self.L3 = nn.Linear(D_width, D_out)
...
class Agent:
    ...
    def __init__(self, sensory_space, action_space, learning_rate, annealing_rate,
        ...
        self.optimizer = torch.optim.Adam(self.off_policy_net.parameters(), self.learning_rate, weight_decay=0.001)
...
for j in range(1, 11):
    ...
    Player = Agent(8, 4, 0.001, 1/(400*100), 0.1, discount=0.99, batch_size=128, replay_memory=200000,
                   update_freq=1000)
    ...
    if n_record_episodes > 0:
        video_recorder = VideoRecorder(env, "./video/Lunar_DQN_untrained.mp4", enabled=True)
    ...
    if n_record_episodes > 0:
        video_recorder = VideoRecorder(env, "./video/Lunar_DQN_trained.mp4", enabled=True)
    for i_episode in range(n_record_episodes):
        observation = env.reset()
    ...
    np.savetxt("LunarLandar_DQN"+str(j)+".csv", res, delimiter=",")
