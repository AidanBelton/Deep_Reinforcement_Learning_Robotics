...
for j in range(0, 10):
    ...
    Player = Agent(8, 4, 0.001, discount=0.99)
    ...
    if n_record_episodes > 0:
        video_recorder = VideoRecorder(env, "./video/Lunar_untrained.mp4", enabled=True)
    for i_episode in range(n_record_episodes):
        ...
        for t in range(episode_length):
            ...
            last_reward = reward/140
            total_reward += reward/140
    ...
    if n_record_episodes > 0:
        video_recorder = VideoRecorder(env, "./video/Lunar_trained.mp4", enabled=True)
    for i_episode in range(n_record_episodes):
        observation = env.reset()
    ...
    np.savetxt("LunarLandar_Policy_Grad"+str(j)+".csv", res, delimiter=",")
